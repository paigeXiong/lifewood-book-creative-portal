...
邮件为可选接入，配置字段见[邮箱验证与邮件通知](./email.md)。在 Linux 上可通过受保护的 `custom.env` 提供邮件变量；Windows 使用实际服务进程可读取的受保护配置。不要仅在开发终端设置变量后假定系统服务已生效，也不要把发件密码写入发布包。重启后，负责人可在“系统配置 → 邮件状态”问号中查看六项配置格式检查；通过后仍需真实收信验收。

多个 OIDC 服务通过后台“登录方式”分别配置，见[企业账号登录](./oidc.md)。正式域名、可信代理和回调配置需一起验证；备份恢复同时保留数据保护密钥，才能继续解密 OIDC 密钥和邮件待发内容。邮件/OIDC 未启用时不影响现有密码登录。
...
新增后台管理入口和独立进程恢复流程见 [后台备份管理](./managed-backups.md)。它通过排空请求与暂停后台写入生成一致性快照，无需停止进程；以下原有脚本仍要求先停机，不应直接用于运行中的数据目录。

服务端运行期间会独占 `data/platform.lock`。任何平台的备份都必须覆盖整个生产数据目录，并确保取得快照期间不能写入；只复制 `platform.db` 会丢失附件、最终成品和登录 Cookie 密钥。
...
未发布的开发版本增加了 Windows 备份目录的私有服务权限准备，以及跨语言安装、覆盖升级和卸载重装的 CI 演练。已有备份树若被改为外部账号所有或含有额外权限，安装器会中止并提示管理员检查。详见 [安装与升级验证](./installer-lifecycle.md)，其中区分了本机已检查内容与待 CI 执行的服务安装项目。
