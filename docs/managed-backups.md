...
平台每分钟检查一次备份状态。异常仅发送给有效的平台负责人，使用已有站内通知队列；不发送邮件。通知规则在“系统配置 → 通知”中维护，收件范围固定为平台负责人。

- 自动备份失败：最新结束的自动备份失败，且之后没有成功备份。
- 备份损坏：完整性校验确认损坏。复检排队、中断、无法读取或结果未保存不会解除已确认的损坏；该归档校验通过或被删除后才解除。
- 缺少近期备份：仅在开启定时备份时检查。每日计划超过 48 小时、每周计划超过 14 天没有成功备份时提醒。时间从首次观察到启用与最近成功备份时间的较晚者算起；关闭计划后重置启用宽限。

同一异常持续存在时不重复投递，去重记录随数据库持久保存。异常解除后原通知显示已处理；再次出现异常会产生新通知。损坏归档集合变化时产生更新提醒。通知正文不包含服务器路径、归档标识或客户资料；点击“查看备份”进入管理页，客户门户内的负责人也会按服务端配置跳转。

去重状态和收件人快照在同一事务中写入。投递继续使用现有失败重试机制；角色变更后，已失去负责人权限的账号也无法读取历史备份告警。数据库不可写或平台停机时站内提醒本身不可用，不能替代外部监控。
