...
- 角色参考图和风格参考图使用与其他项目附件相同的真实文件存储，不保存浏览器临时 URL 或前端伪造占位数据。
- `character-reference` 上传必须携带当前草稿中存在的 `characterId`；`style-reference` 归属创意风格。附件元数据分别保存在角色和创意模型中。
- 删除仍包含参考图的角色前，客户端先通过文件删除接口清理附件；服务端拒绝绕过该流程直接移除角色，避免孤儿文件。
- 下载、管理员查看、草稿删除恢复和异常删除恢复必须统一枚举书籍、角色、风格及第三步参考附件。
- 文件名不得直接作为磁盘路径。
...
- `organizations` 表保存稳定 ID、规范化唯一名称、启用状态和时间戳；`users.organization_id` 为可空的单组织关联。新库使用外键，旧数据库启动迁移只新增表、列、索引和等价完整性触发器，不删除或重建用户与项目数据。
- 管理 API 负责组织新增、重命名、软停用和账号分配；停用组织保留已有归属但拒绝新分配。客户 API 只通过 `CurrentUser.organization` 返回只读组织摘要。
- 当前项目查询仍按稳定用户 ID 隔离。组织归属不自动扩大项目可见范围，直到业务方明确同组织共享边界并补充后端授权规则。
...
- 没有真实头像时，`GET /api/me/avatar` 根据当前用户昵称首字母和稳定配色生成受保护的 SVG 图片响应；该实现不引入反射型或原生图像库，必须继续兼容 Native AOT。
- 前端账户入口只使用 `<img>` 加载头像图片，不使用 HTML 文字与 CSS 背景拼装头像。
- 客户端头像编辑器使用浏览器图片解码与 Canvas 完成拖动、缩放和方形裁剪，并固定输出 512×512 PNG；API 仅存储经过结构、像素和尺寸校验的 PNG，从而不依赖服务端原生图像库并保持 Native AOT 兼容。
