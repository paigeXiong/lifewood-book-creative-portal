...
import { adminService, optionService } from "@lifewood/api-client";
...
describe("character preset editor",()=>{
  it.each(["zh-CN","en-US"] as const)("retains the acknowledged version and selected image after upload failure (%s)",async locale=>{
    ...
    const upload=vi.spyOn(adminService,"uploadCharacterPresetImage").mockRejectedValueOnce(new Error("offline")).mockImplementation(async()=>stored);
    ...
    try{
      ...
      expect(upload).toHaveBeenCalledTimes(2);expect(c.querySelector('[role="dialog"]')).toBeNull();
    }finally{
