...
import { adminService, authService, localizedApiError } from "@lifewood/api-client";
import { isSupportedLocale, localizedPath, setLocale } from "@lifewood/i18n";
import type { AdminProjectDetail, AdminProjectSummary, AdminUser, CurrentUser, ProjectPriority, SupportedLocale, WorkflowStatus } from "@lifewood/domain";
import { FinalDeliveryPanel } from "./FinalDeliveryPanel";

const WORKFLOWS: WorkflowStatus[] = ["new", "contacting", "confirmed", "in_production", "awaiting_customer", "completed", "closed"];
...
function ProjectsPage({ locale }: { locale: SupportedLocale }) {
  ...
  return <main className="content projects-content"><section className="page-toolbar"><form onSubmit={submitSearch} role="search"><input aria-label={t("admin.projects.search")} type="search" value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder={t("admin.projects.search")} /><button>{t("common.search")}</button></form><select aria-label={t("admin.projects.statusFilter")} value={workflow} onChange={(event) => { setPage(1); setWorkflow(event.target.value as WorkflowStatus | ""); }}><option value="">{t("admin.projects.allStatuses")}</option>{WORKFLOWS.map((item) => <option value={item} key={item}>{t(`admin.workflow.${item}`)}</option>)}</select><select aria-label={t("admin.projects.priorityFilter")} value={priority} onChange={(event) => { setPage(1); setPriority(event.target.value as ProjectPriority | ""); }}><option value="">{t("admin.projects.allPriorities")}</option>{PRIORITIES.map((item) => <option value={item} key={item}>{t(`admin.priority.${item}`)}</option>)}</select><span className="result-count">{t("admin.projects.count", { count: projects.data?.total ?? 0 })}</span></section>
    <div className="master-detail"><section className="project-pane" aria-label={t("admin.projects.list")}>
      ...
      <div className="project-rows">{projects.data?.items.map((item) => <ProjectRow key={item.id} item={item} selected={item.id === selectedId} locale={locale} onSelect={() => setSelectedId(item.id)} />)}</div>
    ...
    </section><ProjectDetail detail={detail.data} loading={detail.isPending && Boolean(selectedId)} locale={locale} assignees={assignees} busy={updateWorkflow.isPending || addNote.isPending} error={updateWorkflow.error ?? addNote.error} onWorkflow={(value) => updateWorkflow.mutate(value)} onNote={(body) => addNote.mutate(body)} /></div>
  </main>;
}

function ProjectRow({ item, selected, locale, onSelect }: { item: AdminProjectSummary; selected: boolean; locale: SupportedLocale; onSelect: () => void }) {
  const { t } = useTranslation();
  return <button className={selected ? "project-row selected" : "project-row"} onClick={onSelect}>{item.coverUrl ? <img src={adminAssetUrl(item.id, item.coverUrl)} alt="" /> : <span className="cover-placeholder" />}<span className="row-main"><strong>{item.projectName || item.bookTitle || "—"}</strong><small>{item.ownerName} · {item.bookTitle || "—"}</small><small>{item.taskNumber ?? item.id.slice(0, 8)} · {formatDate(item.updatedAt, locale)}</small></span><span className={`status status-${item.workflowStatus}`}>{t(`admin.workflow.${item.workflowStatus}`)}</span></button>;
}

function ProjectDetail({ detail, loading, locale, assignees, busy, error, onWorkflow, onNote }: { detail?: AdminProjectDetail; loading: boolean; locale: SupportedLocale; assignees: AdminUser[]; busy: boolean; error: unknown; onWorkflow: (value: { workflowStatus: WorkflowStatus; priority: ProjectPriority; assigneeUserId?: string }) => void; onNote: (body: string) => void }) {
  const { t } = useTranslation();
  ...
  const saveNote = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = event.currentTarget; const body = String(new FormData(form).get("body") ?? "").trim(); if (body) { onNote(body); form.reset(); } };
  return <aside className="detail-pane"><div className="detail-title"><div><span className="eyebrow">{task.taskNumber ?? task.id.slice(0, 8)}</span><h2>{task.project.projectName || task.book.title || "—"}</h2><p>{detail.ownerName} · {detail.ownerEmail}</p></div><span className={`status status-${detail.workflowStatus}`}>{t(`admin.workflow.${detail.workflowStatus}`)}</span></div>
    <form className="workflow-form" onSubmit={saveWorkflow} key={`${task.id}-${detail.workflowStatus}-${detail.priority}-${detail.assigneeUserId}`}><label><span>{t("admin.projects.workflow")}</span><select name="workflowStatus" defaultValue={detail.workflowStatus}>{WORKFLOWS.map((item) => <option key={item} value={item}>{t(`admin.workflow.${item}`)}</option>)}</select></label><label><span>{t("admin.projects.priority")}</span><select name="priority" defaultValue={detail.priority}>{PRIORITIES.map((item) => <option key={item} value={item}>{t(`admin.priority.${item}`)}</option>)}</select></label><label><span>{t("admin.projects.assignee")}</span><select name="assigneeUserId" defaultValue={detail.assigneeUserId ?? ""}><option value="">{t("admin.projects.unassigned")}</option>{assignees.map((item) => <option key={item.id} value={item.id}>{item.displayName}</option>)}</select></label><button className="primary" disabled={busy}>{t("common.save")}</button></form>
...
function CreateUserDialog({ busy, error, onClose, onCreate }: { busy: boolean; error: unknown; onClose: () => void; onCreate: (value: { displayName: string; email: string; password: string; role: "customer" | "admin" }) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="create-user-title"><div className="modal-title"><h2 id="create-user-title">{t("admin.users.create")}</h2><button aria-label={t("common.close")} onClick={onClose}>×</button></div><form onSubmit={submit}><label><span>{t("admin.users.name")}</span><input name="displayName" minLength={2} maxLength={100} required autoFocus /></label><label><span>{t("admin.users.email")}</span><input name="email" type="email" maxLength={254} required /></label><label><span>{t("admin.users.password")}</span><input name="password" type="password" minLength={12} maxLength={128} required /><small>{t("admin.users.passwordHint")}</small></label><label><span>{t("admin.users.role")}</span><select name="role" defaultValue="customer"><option value="customer">{t("admin.roles.customer")}</option><option value="admin">{t("admin.roles.admin")}</option></select></label>{Boolean(error) && <div className="message error">{localizedApiError(error, t)}</div>}<div className="modal-actions"><button type="button" onClick={onClose}>{t("common.cancel")}</button><button className="primary" disabled={busy}>{t(busy ? "common.loading" : "admin.users.createAction")}</button></div></form></section></div>;
}
