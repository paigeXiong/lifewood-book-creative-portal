...
export function ChangeOwnPasswordDialog({ onClose }: { onClose: () => void }) {
  ...
  return <ModalFrame labelledBy="change-own-password-title" busy={change.isPending} onClose={onClose}>
    <div className="modal-title"><h2 id="change-own-password-title">{t("admin.account.changePassword")}</h2><button type="button" aria-label={t("common.close")} disabled={change.isPending} onClick={onClose}>×</button></div>
    <form onSubmit={submit} aria-busy={change.isPending}>
      ...
      <div className="modal-actions"><button type="button" disabled={change.isPending} onClick={onClose}>{t("common.cancel")}</button><button className="primary" disabled={change.isPending}>{t(change.isPending ? "admin.account.saving" : "admin.account.save")}</button></div>
    </form>
  </ModalFrame>;
}
...
export function ResetUserPasswordDialog({ user, onClose }: { user: AdminUser; onClose: () => void }) {
  ...
  return <ModalFrame labelledBy="reset-user-password-title" busy={reset.isPending} onClose={onClose}>
    <div className="modal-title"><div><h2 id="reset-user-password-title">{t("admin.users.resetTitle")}</h2><p>{t("admin.users.resetFor", { name: user.displayName })}</p></div><button type="button" aria-label={t("common.close")} disabled={reset.isPending} onClick={onClose}>×</button></div>
    <form onSubmit={submit} aria-busy={reset.isPending}>
      ...
      <div className="modal-actions"><button type="button" disabled={reset.isPending} onClick={onClose}>{t("common.cancel")}</button><button className="primary" disabled={reset.isPending}>{t(reset.isPending ? "admin.account.saving" : "admin.users.resetAction")}</button></div>
    </form>
  </ModalFrame>;
}
