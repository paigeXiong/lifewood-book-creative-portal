...
export function CharacterPresetsPage({ locale, imageBase }: { locale: SupportedLocale; imageBase: string }) {
  ...
  return <main className="content config-content preset-config">
    ...
    <section className="page-toolbar">
      ...
      <span className="result-count">{t("admin.presets.count", { count: presets.data?.length ?? 0 })}</span><HelpPopover label={t("admin.presets.title")}>{t("admin.presets.hint")}</HelpPopover>
  ...
  return <ModalFrame className="preset-modal" labelledBy="preset-title" busy={busy} onClose={requestClose}>
    ...
    <form onSubmit={submit} onChange={markDirty}>
      <fieldset disabled={busy} className="preset-fields">
        <div className="preset-image-row">{(preview || preset.imageUrl) && <img src={preview ?? imageSource(preset.imageUrl,imageBase)} alt={t("admin.presets.image")}/>}<div className="preset-image-controls"><div className="field-help-heading"><strong>{t("admin.presets.image")}</strong><HelpPopover label={t("admin.presets.image")}>{t("admin.presets.imageHint")}</HelpPopover></div><label className="preset-upload"><input type="file" accept=".jpg,.jpeg,.png,.webp" aria-label={t("admin.presets.chooseImage")} onChange={e=>{const file=e.target.files?.[0];e.target.value="";if(!file)return;if(file.size>5000000||!/^image\/(jpeg|png|webp)$/.test(file.type)){setImageError(t("admin.presets.imageInvalid"));setImage(undefined);return;}setImageError(undefined);setImage(file);}}/><span>{t("admin.presets.chooseImage")}</span></label>{image && <span className="preset-file-name">{image.name}</span>}{(image || imageError) && <button type="button" onClick={()=>{setImage(undefined);setImageError(undefined);}}>{t("admin.presets.cancelImage")}</button>}</div></div>
