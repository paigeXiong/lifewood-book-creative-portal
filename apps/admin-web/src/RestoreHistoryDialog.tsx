...
export function RestoreHistoryDialog({locale,userId,busy,downloading,downloadError,onDownload,onClose}: {
  ...
  return <ModalFrame labelledBy="restore-history-title" onClose={onClose} className="restore-history-dialog">
    <div className="modal-title"><h2 id="restore-history-title">{t("restore.history.title")}</h2><button onClick={onClose} aria-label={t("common.close")}>×</button></div>
    ...
    {data&&<div className="backup-pagination"><span>{t("restore.history.total",{count:data.total})}</span><button disabled={data.page<=1||query.isFetching} onClick={()=>setPage(data.page-1)} aria-label={t("backups.previous")}>‹</button><span>{data.page}</span><button disabled={data.page*data.pageSize>=data.total||query.isFetching} onClick={()=>setPage(data.page+1)} aria-label={t("backups.nextPage")}>›</button></div>}
  </ModalFrame>;
}
