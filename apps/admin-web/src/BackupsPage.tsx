...
export function BackupsPage({ locale, userId, allowed }: { locale: SupportedLocale; userId: string; allowed: boolean }) {
  ...
  const downloadState = useExportDownload(JSON.stringify([userId,allowed,locale]));
  const {pending:downloading,error:downloadError} = downloadState;
  ...
  async function download(item: BackupRecord) {
    if(!allowed)return;
    await downloadState.run(signal=>adminService.downloadBackup(item.id,signal),`lifewood-backup-${item.createdAt.slice(0,19).replaceAll(":","-")}.zip`);
  }
  const error = query.error || create.error || verify.error || downloadError;
  return <main className="content config-content pagination-layout"><SettingsTabs locale={locale}/>
    {!allowed ? <p role="alert">{t("backups.forbidden")}</p> : <section className="backup-surface pagination-layout pagination-surface">
      ...
      {downloading&&<p role="status">{t("common.loading")} <button onClick={downloadState.cancel}>{t("common.cancel")}</button></p>}
      {data?.paused&&<p className="message" role="status">{t("backups.paused")}</p>}
      {query.isPending ? <p role="status">{t("common.loading")}</p> : !data?.items.length ? <p className="backup-empty pagination-scroll">{t("backups.empty")}</p> : <div className="backup-table-wrap pagination-scroll"><table className="backup-table"><thead><tr><th>{t("backups.createdAt")}</th><th>{t("backups.source")}</th><th>{t("backups.statusLabel")}</th><th>{t("backups.size")}</th><th>{t("backups.actions")}</th></tr></thead><tbody>{data.items.map(item=><tr key={item.id}>
      ...
      {data&&<div className="backup-pagination pagination-footer"><span>{t("backups.total",{count:data.total})}</span><button disabled={page<=1} onClick={()=>setPage(page-1)} aria-label={t("backups.previous")} data-icon-motion="press"><span aria-hidden="true" data-icon-glyph>‹</span></button><span>{page}</span><button disabled={page*data.pageSize>=data.total} onClick={()=>setPage(page+1)} aria-label={t("backups.nextPage")} data-icon-motion="press"><span aria-hidden="true" data-icon-glyph>›</span></button></div>}
    </section>}
    {showHistory&&allowed&&<RestoreHistoryDialog locale={locale} userId={userId} busy={restoreBusy||!!data?.paused} downloading={!!downloading} downloadError={downloadError} onDownload={item=>void download(item)} onCancelDownload={downloadState.cancel} onClose={()=>setShowHistory(false)}/>}
    {restoring&&<RestoreDialog item={restoring} locale={locale} userId={userId} onClose={()=>setRestoring(undefined)}/>}
    {policy&&<UnsavedFormModal labelledBy="backup-policy-title" busy={save.isPending} onClose={()=>setPolicy(undefined)}>{({markDirty,requestClose})=><form onChange={markDirty} onSubmit={e=>{e.preventDefault();save.mutate(policy);}}><div className="modal-title"><h2 id="backup-policy-title">{t("backups.policy")}</h2><button type="button" onClick={requestClose} disabled={save.isPending} aria-label={t("common.close")} data-icon-motion="press"><span aria-hidden="true" data-icon-glyph>×</span></button></div><div className="backup-policy-fields">
    ...
    </div>{save.error&&<p role="alert">{localizedApiError(save.error,t)}</p>}<div className="modal-actions"><HelpPopover label={t("backups.policy")}>{t("backups.retentionHelp")}</HelpPopover><button type="button" onClick={requestClose} disabled={save.isPending}>{t("common.cancel")}</button><button className="primary" disabled={save.isPending}>{t("common.save")}</button></div></form>}</UnsavedFormModal>}
