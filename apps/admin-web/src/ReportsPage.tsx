...
export function ReportsPage({locale}:{locale:SupportedLocale}){
 ...
 return <main className="content reports-page pagination-layout"><div className="reports-toolbar"><Link to={`/${locale}/workbench`}>{t("operations.workbench")}</Link><button disabled={!query.data||query.isFetching||query.isError||downloadState.pending} onClick={exportCsv}>{t("productivity.exportCsv")}</button></div>
  ...
  {query.isPending?<p role="status">{t("common.loading")}</p>:query.error?<p role="alert">{localizedApiError(query.error,t)}<button onClick={()=>void query.refetch()}>{t("common.retry")}</button></p>:<>
   <div className="pagination-scroll"><div className="reports-metrics">{metrics.map(key=><section key={key}><span>{t("productivity.metrics."+key)}</span><strong>{data.reduce((sum,day)=>sum+day[key],0)}</strong></section>)}</div>
