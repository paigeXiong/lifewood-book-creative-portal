...
import { ApiError, announcementService, captureAccountGuard, localizedApiError } from "@lifewood/api-client";
...
export function AnnouncementsPage({locale,userId}:{locale:SupportedLocale;userId:string}) {
 ...
 const launchKey=useRef<string|undefined>(undefined);
 useEffect(()=>{
   const params=new URLSearchParams(location.search);
   if(params.get("create")!=="scheduled"||launchKey.current===location.key)return;
   launchKey.current=location.key;params.delete("create");
   const next=editorRef.current??{...empty(userId),scheduleAfterSave:true};
 ...
 },onSuccess:(_data,saved)=>{closeNoticeDraft(saved);if(alive.current && editorRef.current?.flowId===saved.flowId){setEditor(undefined);if(saved.scheduleAfterSave){setScheduledItem(_data);setReturnToTasks(true);}}void client.invalidateQueries({queryKey:["admin-announcements"]});}});
 ...
 const [scheduledItem,setScheduledItem]=useState<AnnouncementDocument>();
 const [returnToTasks,setReturnToTasks]=useState(false);
 ...
 {list.data?.pages.slice(0,pages).flatMap(p=>p.items).map(d=><tr key={d.id}><td data-label={t("announcements.title")}><strong>{noticeTitle(d.content,locale)}</strong><small>{t(`announcements.${d.content.placement}`)}</small></td><td data-label={t("announcements.audience")}>{t(`announcements.${d.content.audience}`)}<small>{d.content.languages.join(" · ")}{d.content.organizationIds.length>0?` · ${t("announcements.organizations")} (${d.content.organizationIds.length})`:""}</small></td><td data-label={t("announcements.status")}>{t(`announcements.${d.status}`)}{d.scheduledAt&&<small>{new Intl.DateTimeFormat(locale,{dateStyle:"medium",timeStyle:"short"}).format(new Date(d.scheduledAt))}</small>}</td><td data-label={t("announcements.recipients")}>{d.content.placement!=="login"?d.recipients:"—"}</td><td data-label={t("announcements.actions")}><div className="notice-actions"><button disabled={busy} onClick={()=>open(d)}>{t(d.status==="draft"?"announcements.edit":"announcements.copy")}</button>{d.status==="draft"&&<button disabled={busy} onClick={()=>{setReturnToTasks(false);setScheduledItem(d);}}>{t("automation.schedule")}</button>}{d.status==="scheduled"&&<button disabled={busy||cancelSchedule.isPending} onClick={()=>{const guard=captureAccountGuard();void confirm(t("automation.cancelConfirm"),false).then(ok=>{if(ok&&alive.current){guard();cancelSchedule.mutate(d);}}).catch(()=>{});}}>{t("automation.cancelSchedule")}</button>}{d.status!=="withdrawn"&&<button disabled={busy} onClick={()=>void actions.run(d,d.status==="published"?"withdraw":"publish")}>{t(d.status==="published"?"announcements.withdraw":"announcements.publish")}</button>}{d.status==="draft"&&<button className="danger" disabled={busy} onClick={()=>void actions.run(d,"delete")}>{t("announcements.deleteDraft")}</button>}</div></td></tr>)}
 ...
 {scheduledItem&&<AnnouncementScheduleDialog item={scheduledItem} locale={locale} onClose={()=>setScheduledItem(undefined)} onSaved={()=>{setScheduledItem(undefined);void list.refetch();if(returnToTasks)navigate(`/${locale}/automation`);}}/>}
 ...
 <fieldset disabled={savePending} className="notice-form"><div className="notice-pair"><label>{t("announcements.placement")}<select value={editor.content.placement} onChange={e=>update({placement:e.target.value as "login"|"personal"|"banner",organizationIds:[],audience:"all",languages:[]})}><option value="banner">{t("announcements.banner")}</option><option value="personal">{t("announcements.personal")}</option><option value="login">{t("announcements.login")}</option></select></label><label>{t("announcements.audience")}<select value={editor.content.audience} onChange={e=>update({audience:e.target.value as "all"|"specified",languages:[],organizationIds:[]})}><option value="all">{t("announcements.all")}</option><option value="specified">{t("announcements.specified")}</option></select></label></div>
 ...
 {editor.content.audience==="specified"&&<section className="notice-target"><div className="field-help-heading"><strong>{t("announcements.languages")}</strong><HelpPopover label={t("announcements.languages")}>{t("announcements.scopeHint")}</HelpPopover></div><div className="notice-actions">{["zh-CN","en-US"].map(l=><label key={l}><input type="checkbox" checked={editor.content.languages.includes(l)} onChange={e=>update({languages:e.target.checked?[...editor.content.languages,l]:editor.content.languages.filter(x=>x!==l)})}/>{l==="zh-CN"?"简体中文":"English"}</label>)}</div>{editor.content.placement!=="login"&&<><strong>{t("announcements.organizations")}</strong><p>{editor.content.organizationIds.map(id=>editor.organizations[id]).filter(Boolean).join(" · ") || (editor.content.organizationIds.length ? `${t("announcements.selected")} · ${editor.content.organizationIds.length}` : "")||t("announcements.noSelection")}</p><div className="notice-actions"><button type="button" onClick={()=>navigate(`/${locale}/organizations?pick=announcement`,{state:{announcementEditor:editor}})}>{t("announcements.chooseOrgs")}</button><button type="button" onClick={()=>update({organizationIds:[]})}>{t("announcements.clear")}</button></div></>}</section>}
