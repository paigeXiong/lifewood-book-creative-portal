...
export function AnnouncementsPage({locale,userId}:{locale:SupportedLocale;userId:string}) {
 ...
 <section className="notice-copy-fields"><label>{t("announcements.contentTitle")}<input required maxLength={160} value={noticeTitle(editor.content,locale)} onChange={e=>update({title:e.target.value})}/></label><label>{t("announcements.contentBody")}<textarea required maxLength={12000} rows={8} value={noticeBody(editor.content,locale)} onChange={e=>update({body:e.target.value})}/></label><p className="muted">{t("announcements.singleContentHint")}</p>{editor.content.titleZh != null && editor.content.titleEn != null && <p className="muted">{t("announcements.legacyContentHint")}</p>}</section>
 <label className="notice-duration">{t("announcements.displayDays")}<input type="number" inputMode="numeric" required min={1} max={3650} step={1} value={editor.content.displayDays??""} onChange={e=>update({displayDays:e.target.value===""?null:Number(e.target.value)})}/></label><p className="muted">{t("announcements.displayDaysHint")}</p>
