...
function TemplateWorkspace({ locale }: { locale: SupportedLocale }) {
  ...
  return <section className="panel mail-preview mail-template-page" aria-label={t("mailEditor.title")}>
    ...
    <div className="mail-preview-controls">
      <label>{t("mailPreview.kind")}<select disabled={editing} value={kind} onChange={e => setKind(e.target.value)}>{(query.data ?? []).map(item => <option key={item.kind} value={item.kind}>{item.subject}</option>)}</select></label>
      <label>{t("mailPreview.language")}<select disabled={editing} value={language} onChange={e => setLanguage(e.target.value as SupportedLocale)}><option value="zh-CN">简体中文</option><option value="en-US">English</option></select></label>
    ...
    {query.isPending ? <p role="status">{t("common.loading")}</p> : query.isError && (!selected || denied) ? <p role="alert">{localizedApiError(query.error, t)} <button onClick={() => void query.refetch()} disabled={query.isFetching}>{t("common.retry")}</button></p> : selected && <div className="mail-template-workspace">
      <TemplateEditor key={`${kind}:${language}`} selected={selected} language={language} onEditing={setEditing} onSaved={async () => { await query.refetch(); }} />
      <div className="mail-template-result"><p className="mail-preview-subject">{t("mailPreview.subject")}：{selected.subject}</p>
      {format === "html" ? <FormattedPreview document={document!} title={t("mailEditor.title")} /> : <pre lang={language}>{selected.body.text}</pre>}
      </div>
    </div>}
  </section>;
}

function TemplateEditor({selected,language,onSaved,onEditing}:{selected:Template;language:SupportedLocale;onSaved:()=>Promise<void>;onEditing:(value:boolean)=>void}) {
  const {t}=useTranslation();
  const [editing,updateEditing]=useState(false);
  const setEditing=(value:boolean)=>{updateEditing(value);onEditing(value);};
  ...
  return <div className="mail-template-editor">
    ...
    {!editing ? <div className="mail-template-actions"><span>{t(selected.enabled?"mailEditor.enabled":"mailEditor.disabled")}</span><button type="button" onClick={()=>{setRevision(selected.revision);setSubject(selected.subject);setIntroduction(selected.introduction);setEnabled(selected.enabled);save.reset();setEditing(true);}}>{t("mailEditor.edit")}</button><button type="button" disabled={test.isPending} onClick={()=>{if(window.confirm(t("mailEditor.testConfirm")))test.mutate();}}>{t("mailEditor.test")}</button></div> : <form onSubmit={e=>{e.preventDefault();save.mutate(false);}}>
      <label>{t("mailPreview.subject")}<input required maxLength={160} value={subject} disabled={save.isPending} onChange={e=>setSubject(e.target.value)}/></label>
      <label>{t("mailEditor.body")}<textarea required maxLength={4000} rows={4} value={introduction} disabled={save.isPending} onChange={e=>setIntroduction(e.target.value)}/></label>
      <div className="mail-template-actions">{selected.kind==="notice"?<label><input type="checkbox" checked={enabled} disabled={save.isPending} onChange={e=>setEnabled(e.target.checked)}/>{t("mailEditor.enable")}</label>:<span>{t("mailEditor.required")}</span>}<HelpPopover label={t("mailEditor.title")}>{t("mailEditor.help")}</HelpPopover></div>
      {save.isError&&<p role="alert">{localizedApiError(save.error,t)}</p>}
      <div className="mail-template-actions"><button type="submit" disabled={save.isPending}>{t("mailEditor.save")}</button><button type="button" disabled={save.isPending} onClick={()=>{if(window.confirm(t("mailEditor.resetConfirm")))save.mutate(true);}}>{t("mailEditor.reset")}</button><button type="button" disabled={save.isPending} onClick={()=>setEditing(false)}>{t("common.cancel")}</button></div>
    </form>}
  </div>;
}
