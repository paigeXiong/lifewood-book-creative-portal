        ...
        <table className="organizations-table">
          ...
          <tbody>{organizations.data?.items.map((organization) => (
            <Fragment key={organization.id}><tr>
              ...
              <td data-label={t("admin.organizations.action")}>{picking?<label><input type="checkbox" disabled={noticeSaving || !organization.active || !selected.includes(organization.id)&&selected.length>=200} checked={selected.includes(organization.id)} onChange={e=>selectOrganization(organization,e.target.checked)}/>{t("announcements.select")}</label>:<div className="row-actions"><button type="button" onClick={() => setEditing(organization)}>{t("admin.organizations.edit")}</button>{organization.active&&<button type="button" onClick={()=>navigate(`/${locale}/invitations?organization=${organization.id}`)}>{t("invitation.title")}</button>}</div>}</td>
