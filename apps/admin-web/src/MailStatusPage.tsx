...
export function MailStatusPage({ locale, allowed }: { locale: SupportedLocale; allowed: boolean }) {
  ...
  return <main className="content config-content"><SettingsTabs locale={locale} />
    {!allowed ? <p role="alert">{t("mailQueue.ownerOnly")}</p> : <section className="panel mail-status">
      <div className="mail-status-toolbar">
        {data && <div className="mail-config-state"><span role="status">{t(data.available ? "mailQueue.available" : "mailQueue.unavailable")}</span>{!!data.configurationChecks?.length && <HelpPopover label={t("mailQueue.configuration.title")}>
      ...
      {query.isPending ? <p role="status">{t("common.loading")}</p> : data && <>
        <div className="mail-status-counts">{data.counts.map(item => <button key={item.status} type="button" aria-pressed={status === item.status} onClick={() => filter("status", status === item.status ? "" : item.status)}>{t(`mailQueue.states.${item.status}`)} <strong>{item.count.toLocaleString(locale)}</strong></button>)}</div>
        <div className="mail-status-toolbar">
