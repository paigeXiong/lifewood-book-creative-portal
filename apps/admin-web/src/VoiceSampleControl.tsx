    ...
    <div className="voice-sample-line">
      {src ? <>
        ...
        <button type="button" className="voice-play" disabled={busy && !playing && !waiting} onClick={() => void toggle()}
          aria-label={t(failed ? "admin.voices.retrySample" : playing || waiting ? "admin.voices.pauseSample" : "admin.voices.playSample", { name })}>
          <span aria-hidden="true">{failed ? "↻" : playing || waiting ? "Ⅱ" : "▶"}</span>
        </button>
      ...
      {src && <div className="voice-sample-more" ref={menu}>
        <button type="button" ref={more} disabled={busy} aria-label={t("admin.voices.moreAudio", { name })} aria-expanded={open} aria-controls={menuId} onClick={() => setOpen(value => !value)}>⋯</button>
