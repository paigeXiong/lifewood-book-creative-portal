...
export function ProjectReturns({id, version, status, workflowUpdatedAt, locale, renderSnapshot}: {id:string;version:number;status:string;workflowUpdatedAt:string;locale:SupportedLocale;renderSnapshot:(snapshot:string)=>ReactNode}) {
  ...
  return <section className="detail-section revision-admin">
    <button className="primary" disabled={status!=="submitted"||save.isPending} onClick={()=>{save.reset();setExpected({version,workflowUpdatedAt});setEditing(true);}}>{labels.return}</button>
