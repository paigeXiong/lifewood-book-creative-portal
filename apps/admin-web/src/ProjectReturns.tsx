...
export function ProjectReturns({id, version, status, workflowUpdatedAt, locale, renderSnapshot, canReturn = true, canReply = true}: {canReturn?:boolean;canReply?:boolean;id:string;version:number;status:string;workflowUpdatedAt:string;locale:SupportedLocale;renderSnapshot:(snapshot:string)=>ReactNode}) {
  ...
  return <section className="detail-section revision-admin">
    <ProjectAction slot="returns"><button disabled={!canReturn||status!=="submitted"||save.isPending} onClick={()=>{save.reset();setExpected({version,workflowUpdatedAt});setEditing(true);}}>{labels.return}</button></ProjectAction>
    {editing&&<ModalFrame labelledBy="return-title" busy={save.isPending} onClose={()=>setEditing(false)}><form className="revision-return-form" onSubmit={e=>{e.preventDefault();save.mutate();}}><header><div className="revision-dialog-icon" aria-hidden="true">↶</div><h2 id="return-title">{labels.return}</h2><button type="button" className="revision-dialog-close" aria-label={labels.close} disabled={save.isPending} onClick={()=>setEditing(false)} data-icon-motion="press"><span aria-hidden="true" data-icon-glyph>×</span></button></header><div className="revision-reason-list">
    ...
    {rounds.length>0&&<><h3>{labels.history}</h3>{rounds.map(round=><details id={`notification-${round.id}`} key={round.id} open={!round.submittedAt}><summary>{new Date(round.createdAt).toLocaleString(locale)} · {round.submittedAt?labels.submitted:labels.pending}</summary>
      {round.submittedAt&&<RevisionDiff before={round.beforeSnapshot} after={round.afterSnapshot} locale={locale} projectId={id} units={units}/>}
