...
import type { ConfigOption, ReferenceCategory, TaskDraft, VoiceReference } from "@lifewood/domain";
...
function VoiceSamplesSection({ form, voices, maxSelected, playingVoice, voiceTagMap, onSelect, onPrefer, onToggleAudio }: { form: UseFormReturn<VoiceFormValues>; voices: VoiceReference[]; maxSelected: number; playingVoice?: string; voiceTagMap: Map<string, string>; onSelect: (id: string, selected: boolean) => void; onPrefer: (id: string) => void; onToggleAudio: (voice: VoiceReference) => void }) {
  ...
  return <section className="form-panel voice-samples"><div className="section-heading"><div><h2 id="voice-samples-heading"><span>3.2</span>{t("voice.sections.samples")}</h2><p>{t("voice.sampleDisclaimer")}</p></div><span className="selection-count" aria-live="polite">{t("voice.selectedCount", { count: selectedVoiceIds.length, max: maxSelected })}</span></div>
    ...
    <Field className="field-wide" label={t("voice.fields.customVoice")} htmlFor="custom-voice" error={form.formState.errors.customVoiceDescription?.message}><textarea id="custom-voice" rows={2} maxLength={300} autoComplete="off" {...form.register("customVoiceDescription")} /></Field>
  </section>;
}

function ReferencesSection({ form, categories, locale, uploadCategory, transfers, uploadError, onUpload, onRemoveAsset, onCancelUpload, onRetryUpload }: { form: UseFormReturn<VoiceFormValues>; categories: ReferenceCategory[]; locale: string; uploadCategory?: string; transfers: TransferItem[]; uploadError?: string; onUpload: (category: ReferenceCategory, files: FileList | null) => Promise<void>; onRemoveAsset: (id: string) => Promise<void>; onCancelUpload: (id: string) => void; onRetryUpload: (item: TransferItem) => Promise<void> }) {
  const { t } = useTranslation();
  ...
  return <section className="form-panel reference-section"><div className="section-heading"><div><h2><span>3.3</span>{t("voice.sections.references")}</h2><p>{t("voice.referenceHint")}</p></div></div>
    <div className="upload-grid">{categories.map((category) => { const files = assets.filter((asset) => asset.categoryId === category.id); return <div className="upload-card" key={category.id}><div><strong>{category.label}</strong><small>{category.description}</small><small>{t("voice.fileLimit", { size: formatBytes(category.maxBytes, locale), count: category.maxFiles })}</small></div><label className={`button button-secondary ${uploadCategory ? "disabled" : ""}`} htmlFor={`upload-${category.id}`} aria-label={t("voice.chooseFor", { category: category.label })}>{uploadCategory === category.id ? t("voice.uploading") : t("voice.chooseFiles")}</label><input id={`upload-${category.id}`} className="visually-hidden" type="file" multiple={category.maxFiles > 1} accept={category.accept.join(",")} disabled={Boolean(uploadCategory) || files.length >= category.maxFiles} onChange={(event) => { void onUpload(category, event.target.files); event.target.value = ""; }} />{files.length > 0 && <ul className="uploaded-files">{files.map((asset) => <li key={asset.id}><span title={asset.fileName}>{asset.fileName}</span><small>{formatBytes(asset.sizeBytes, locale)}</small><button type="button" aria-label={t("voice.removeFile", { fileName: asset.fileName })} onClick={() => void onRemoveAsset(asset.id)}>{t("voice.remove")}</button></li>)}</ul>}</div>; })}</div>
...
export function VoiceAndReferencesPage() {
  ...
  const save = useMutation({
    mutationFn: async ({ values, continueAfter }: { values: VoiceFormValues; continueAfter: boolean }) => {
      ...
      return { saved: await projectService.saveVoiceAndReferences(current.id, next, validLocale, continueAfter), continueAfter };
    },
    onSuccess: ({ saved, continueAfter }) => { queryClient.setQueryData(["project", taskId], saved); void queryClient.invalidateQueries({ queryKey: ["projects"] }); form.reset(form.getValues()); setSaveState("saved"); if (continueAfter) navigate(localizedPath(validLocale, `/tasks/${saved.id}/edit/review`)); },
  ...
  const retryUpload = async (item: TransferItem) => { if (uploadingRef.current) return; uploadingRef.current = true; setUploadCategory(item.categoryId); await uploadOne(item); uploadingRef.current = false; setUploadCategory(undefined); };
  ...
  const removeAsset = async (id: string) => {
    if (!window.confirm(t("voice.removeConfirm"))) return;
    try { const current = queryClient.getQueryData<TaskDraft>(["project", taskId]) ?? draftQuery.data!; const saved = await projectService.deleteAsset(taskId, id, current.version, validLocale); queryClient.setQueryData(["project", taskId], saved); form.setValue("assets", saved.voiceAndReferences.assets, { shouldDirty: true }); }
  ...
  const continueStep = form.handleSubmit((values) => {
    ...
    save.mutate({ values: checked.data, continueAfter: true });
  });

  return <div className="wizard-page voice-page">
    <StepProgress current={3} />
    <div className="wizard-heading"><div><h1>{t("wizard.steps.voice")}</h1><p>{t("voice.intro")}</p></div><span className={`save-state save-${saveState}`} aria-live="polite">{statusText}</span></div>
      ...
      <div className="voice-layout"><div className="form-stack">
        <section className="form-panel voice-settings"><div className="section-heading"><div><h2><span>3.1</span>{t("voice.sections.settings")}</h2><p>{t("voice.settingsHint")}</p></div></div>
          <div className="form-grid voice-grid">
            <Field label={t("voice.fields.contentLanguage")} htmlFor="voice-content-language" required error={form.formState.errors.contentLanguageId?.message}><select id="voice-content-language" autoComplete="off" {...form.register("contentLanguageId")}><option value="" /><Options items={options.contentLanguages} /></select></Field>
            <Field label={t("voice.fields.narrationTone")} htmlFor="narration-tone" required error={form.formState.errors.narrationToneId?.message}><select id="narration-tone" autoComplete="off" {...form.register("narrationToneId")}><option value="" /><Options items={options.narrationTones} /></select></Field>
            <Field label={t("voice.fields.speechRate")} htmlFor="speech-rate" required error={form.formState.errors.speechRateId?.message}><select id="speech-rate" autoComplete="off" {...form.register("speechRateId")}><option value="" /><Options items={options.speechRates} /></select></Field>
            <Field label={t("voice.fields.voiceGender")} htmlFor="voice-gender"><select id="voice-gender" autoComplete="off" {...form.register("voiceGenderId")}><option value="" /><Options items={options.voiceGenders} /></select></Field>
            <Field label={t("voice.fields.voiceAge")} htmlFor="voice-age"><select id="voice-age" autoComplete="off" {...form.register("voiceAgeId")}><option value="" /><Options items={options.voiceAges} /></select></Field>
            <Field label={t("voice.fields.accent")} htmlFor="voice-accent"><select id="voice-accent" autoComplete="off" {...form.register("accentId")}><option value="" /><Options items={options.accents} /></select></Field>
            <Field label={t("voice.fields.emotionStyle")} htmlFor="voice-emotion"><select id="voice-emotion" autoComplete="off" {...form.register("emotionStyleId")}><option value="" /><Options items={options.voiceEmotions} /></select></Field>
            <Field className="field-wide" label={t("voice.fields.pronunciationNotes")} htmlFor="pronunciation-notes" error={form.formState.errors.pronunciationNotes?.message}><textarea id="pronunciation-notes" rows={2} maxLength={200} autoComplete="off" {...form.register("pronunciationNotes")} /></Field>
          </div>
        </section>
        ...
        <ReferencesSection form={form} categories={options.referenceCategories} locale={validLocale} uploadCategory={uploadCategory} transfers={transfers} uploadError={uploadError} onUpload={upload} onRemoveAsset={removeAsset} onCancelUpload={cancelUpload} onRetryUpload={retryUpload} />
        ...
        <section className="form-panel direction-section"><div className="section-heading"><div><h2><span>3.4</span>{t("voice.sections.direction")}</h2><p>{t("voice.directionHint")}</p></div></div><div className="form-grid">
          <Field className="field-wide" label={t("voice.fields.coreMessage")} htmlFor="core-message" required error={form.formState.errors.coreMessage?.message}><textarea id="core-message" rows={3} maxLength={300} autoComplete="off" {...form.register("coreMessage")} /></Field>
          <Field label={t("voice.fields.requiredScenes")} htmlFor="required-scenes" error={form.formState.errors.requiredScenes?.message}><textarea id="required-scenes" rows={3} maxLength={300} autoComplete="off" {...form.register("requiredScenes")} /></Field>
          <Field label={t("voice.fields.authorPreferences")} htmlFor="author-preferences" error={form.formState.errors.authorPreferences?.message}><textarea id="author-preferences" rows={3} maxLength={300} autoComplete="off" {...form.register("authorPreferences")} /></Field>
          <Field label={t("voice.fields.closingMessage")} htmlFor="closing-message" error={form.formState.errors.closingMessage?.message}><textarea id="closing-message" rows={3} maxLength={300} autoComplete="off" {...form.register("closingMessage")} /></Field>
          <Field label={t("voice.fields.musicMood")} htmlFor="music-mood" error={form.formState.errors.musicMood?.message}><textarea id="music-mood" rows={2} maxLength={200} autoComplete="off" {...form.register("musicMood")} /></Field>
          <Field className="field-wide" label={t("voice.fields.avoidContent")} htmlFor="avoid-content" error={form.formState.errors.avoidContent?.message}><textarea id="avoid-content" rows={2} maxLength={200} autoComplete="off" {...form.register("avoidContent")} /></Field>
        </div></section>
      </div><VoiceSummary control={form.control} voices={voices} /></div>
      <div className="sticky-actions"><Link className="button button-secondary" to={localizedPath(validLocale, `/tasks/${taskId}/edit/characters`)} onClick={guardLink}>{t("common.back")}</Link><div><button className="button button-secondary" type="button" disabled={save.isPending || Boolean(uploadCategory)} onClick={form.handleSubmit((values) => save.mutate({ values, continueAfter: false }))}>{save.isPending ? t("common.saving") : t("common.saveDraft")}</button>{conflict && <button className="button button-secondary" type="button" onClick={() => { form.reset(); void draftQuery.refetch(); }}>{t("common.reload")}</button>}<button className="button button-primary" type="submit" disabled={save.isPending || Boolean(uploadCategory)}>{save.isPending ? t("common.saving") : t("voice.continueToReview")}<span aria-hidden="true">→</span></button></div></div>
    </form>
  </div>;
}
