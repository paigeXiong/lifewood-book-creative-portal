          ...
          <div className="form-stack">
            ...
            <section className="form-panel book-info-panel">
              ...
              <details className="optional-details" open={Boolean(form.formState.errors.sellingPoint || form.formState.errors.synopsis || form.watch("sellingPoint") || form.watch("synopsis"))}><summary>{t("clientUx.optionalBook")}</summary><div className="form-grid">
                <Field label={t("wizard.fields.sellingPoint")} icon={<FieldIcon name="highlight" />} htmlFor="sellingPoint" className="field-wide" error={form.formState.errors.sellingPoint?.message} hint={<span className={(form.watch("sellingPoint")?.length ?? 0) >= 135 ? "character-count near-limit" : "character-count"}>{t("formFeedback.characters", {count:form.watch("sellingPoint")?.length ?? 0,max:150})}</span>}><textarea id="sellingPoint" rows={2} maxLength={150} {...form.register("sellingPoint")} /></Field>
                <Field label={t("wizard.fields.synopsis")} icon={<FieldIcon name="summary" />} htmlFor="synopsis" className="field-wide" error={form.formState.errors.synopsis?.message} hint={<span className={(form.watch("synopsis")?.length ?? 0) >= 540 ? "character-count near-limit" : "character-count"}>{t("formFeedback.characters", {count:form.watch("synopsis")?.length ?? 0,max:600})}</span>}><textarea id="synopsis" rows={4} maxLength={600} {...form.register("synopsis")} /></Field>
              </div></details>
            </section>
          </div>
