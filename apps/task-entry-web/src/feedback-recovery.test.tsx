...
for(const locale of ["zh-CN","en-US"] as const){
 it(`preserves feedback on timeout and retries the same payload (${locale})`,async()=>{
  await i18n.changeLanguage(locale);const client=new QueryClient({defaultOptions:{queries:{staleTime:Infinity,retry:false}}});client.setQueryData(["feedback-catalog",locale],{categories:[{id:"bug",label:"Bug"}],statuses:[],screenshotMaxBytes:1000000,screenshotSourceMaxBytes:10000000});
  ...
  try{
   await act(async()=>root.render(<QueryClientProvider client={client}><FeedbackButton locale={locale}/></QueryClientProvider>));await act(async()=>host.querySelector<HTMLButtonElement>(".feedback-trigger")!.click());
 ...
 it(`resets the file input and retains the prior screenshot if replacement fails (${locale})`,async()=>{
  await i18n.changeLanguage(locale);const client=new QueryClient({defaultOptions:{queries:{staleTime:Infinity}}});client.setQueryData(["feedback-catalog",locale],{categories:[{id:"bug",label:"Bug"}],statuses:[]});const host=document.createElement("div");document.body.append(host);const root=createRoot(host);
  ...
  try{
   await act(async()=>root.render(<QueryClientProvider client={client}><FeedbackButton locale={locale}/></QueryClientProvider>));await act(async()=>host.querySelector<HTMLButtonElement>(".feedback-trigger")!.click());const input=host.querySelector<HTMLInputElement>('input[type="file"]')!;
