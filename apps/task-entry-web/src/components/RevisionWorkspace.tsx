...
export function RevisionWorkspace({children}: {children: ReactNode}) {
  ...
  useEffect(()=>{setOpen(Boolean(round&&!round.submittedAt&&round.messages.length&&!window.matchMedia?.("(max-width: 900px)").matches));setUnit("");setBody("");},[taskId,round?.id]);
  const selected=round?.reasons.some(r=>r.unit===unit)?unit:round?.reasons[0]?.unit??"";
  const send=useMutation({mutationFn:()=>revisionService.reply(taskId!,round!.id,{id:createId(),unit:selected,body},lang),onSuccess:value=>{client.setQueryData(key,value);setBody("");}});
  ...
  return <RevisionNavigation.Provider value={round&&!round.submittedAt?[...round.reasons.map(r=>r.unit),"review"]:null}>
    ...
    {showChat&&round&&<aside className={`revision-float ${open?"is-open":""}`}>
      ...
      {open&&<section className="revision-chat" aria-label={data!.labels.title}>
        <header><strong>{round.submittedAt?data!.labels.submitted:data!.labels.pending}</strong><button type="button" onClick={()=>setOpen(false)} aria-label={data!.labels.close}>×</button></header>
        <div className="revision-unit-picker">
        {round.reasons.length>1 ? <select aria-label={t("clientUx.requestedUnits")} value={selected} onChange={e=>{if(body.trim()&&!window.confirm(t("wizard.unsavedChanges")))return;setUnit(e.target.value);setBody("");send.reset();}}>{round.reasons.map(r=><option key={r.unit} value={r.unit}>{data!.units.find(u=>u.id===r.unit)?.label}</option>)}</select> : <strong>{data!.units.find(u=>u.id===selected)?.label}</strong>}
        <Link to={step==="review"?"/"+lang+"/tasks/"+taskId+"/edit/"+selected:"/"+lang+"/tasks/"+taskId+"/edit/review"} onClick={e=>{if(document.body.dataset.unsavedChanges==="true"&&!window.confirm(t("wizard.unsavedChanges")))e.preventDefault();}}>{step==="review"?t("review.edit"):data!.labels.review}</Link>
        </div>
        ...
        {!round.submittedAt&&<form onSubmit={e=>{e.preventDefault();if(body.trim()&&!send.isPending)send.mutate();}}><textarea aria-label={data!.labels.reply} placeholder={data!.labels.reply} maxLength={2000} value={body} onChange={e=>setBody(e.target.value)}/>{send.isError&&<p role="alert">{localizedApiError(send.error,t)}</p>}<button className="button" disabled={!body.trim()||send.isPending}>{data!.labels.send}</button></form>}
      </section>}
    </aside>}
    </div>
