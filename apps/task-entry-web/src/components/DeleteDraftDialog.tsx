...
export function DeleteDraftDialog({ title, busy, error, onConfirm, onClose, fallbackFocusRef }: {
  title: string;
  busy: boolean;
  error?: string;
  onConfirm: () => void;
  ...
  return <dialog ref={dialogRef} className="delete-draft-dialog" aria-labelledby={`${id}-title`} aria-describedby={`${id}-description`} aria-busy={busy} onCancel={event => { event.preventDefault(); if (!busy) onClose(); }}>
    <div className="delete-draft-heading">
      ...
      <h2 id={`${id}-title`}>{t("tasks.deleteDialogTitle")}</h2>
    </div>
    <div id={`${id}-description`}>
      <p className="delete-draft-project">{title}</p>
      <p className="delete-draft-warning">{t("tasks.deleteDialogDescription")}</p>
    </div>
    ...
    <div className="delete-draft-actions">
      ...
      <button className="button delete-draft-confirm" type="button" disabled={busy} onClick={onConfirm}>{t(busy ? "tasks.deletingDraft" : "tasks.deleteDraft")}</button>
    </div>
  </dialog>;
}
