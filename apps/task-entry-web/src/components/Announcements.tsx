...
export function Announcements({locale,userId,historyOnly=false,entryLabel}:{locale:SupportedLocale;userId?:string;historyOnly?:boolean;entryLabel?:string}) {
 const {t}=useTranslation();const client=useQueryClient();const headingId=useId();
 const [mode,setMode]=useState<"new"|"history">(historyOnly?"history":"new");const [open,setOpen]=useState(false);const offered=useRef(false);
 ...
 useEffect(()=>{if(!historyOnly && !offered.current && feed.isSuccess && feed.isFetchedAfterMount && !feed.isFetching){offered.current=true;if(items.length)setOpen(true);}},[historyOnly,feed.isSuccess,feed.isFetchedAfterMount,feed.isFetching,items.length]);
