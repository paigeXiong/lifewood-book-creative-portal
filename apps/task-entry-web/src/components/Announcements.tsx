...
export function Announcements({locale,userId}:{locale:SupportedLocale;userId?:string}) {
 ...
 return <>{userId&&<button className="announcement-entry" type="button" onClick={()=>{offered.current=true;close.reset();setMode("history");setOpen(true);}} aria-label={t("announcements.readHistory")}>{t("announcements.title")}</button>}
 ...
 <header><h2 id="announcement-heading">{t(mode==="history"?"announcements.history":"announcements.newNotices")}</h2><button type="button" disabled={close.isPending} onClick={()=>close.mutate()}>{t("announcements.close")}</button></header>
 ...
 <footer><button className="button primary" type="button" disabled={close.isPending} onClick={()=>close.mutate()}>{t(close.isPending?"announcements.loading":userId?"announcements.dismiss":"announcements.close")}</button></footer>
 </dialog></>;
}
