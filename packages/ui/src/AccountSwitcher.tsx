...
export function AccountSwitcher({user,destination}:{user:CurrentUser;destination:(user:CurrentUser)=>string}) {
  ...
  },onError:()=>client.invalidateQueries({queryKey:['saved-accounts',user.id]}),onSuccess:next=>{setPassword('');client.clear();window.location.replace(destination(next));}});
  ...
  const clear=useMutation({mutationFn:authService.clearAccounts,onSuccess:()=>{client.clear();window.location.reload();}});
  const busy=switcher.isPending||remove.isPending||clear.isPending;
  ...
  return <section className="account-switcher" aria-label={t('accountSwitch.title')}>
      ...
      <ul className="saved-account-list">{accounts.data?.items.filter(account=>!account.current).map(account=><li key={account.id}>
        <button className="saved-account-select" type="button" disabled={busy} aria-label={t('accountSwitch.switchNamed',{name:account.displayName})} onClick={()=>{if(account.requiresLogin){setEmail(account.email??'');setPassword('');switcher.reset();setAdding(true);}else void run(account.id);}}>
          <span className="saved-account-initial" aria-hidden="true">{account.displayName.slice(0,1)}</span><span><strong>{account.displayName}</strong><small>{account.email}</small>{account.requiresLogin&&<small>{t("accountSwitch.needsLogin")}</small>}</span>
        </button>
      ...
      {(switcher.error||remove.error||clear.error)&&!adding&&<p role="alert">{localizedApiError(switcher.error??remove.error??clear.error,t)}</p>}
      ...
      <button className="account-switch-add" type="button" disabled={busy} onClick={async()=>{if(await confirm(t("accountSwitch.clearConfirm"),false))clear.mutate();}}>{t("accountSwitch.clearAll")}</button>
