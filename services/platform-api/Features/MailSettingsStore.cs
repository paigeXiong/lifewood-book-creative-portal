...
public sealed record MailSettingsDocument(string Revision, bool Enabled, string Host, int Port, string From, string Username, string ProtectedPassword, string PublicUrl);
public sealed record SaveMailSettings(string Revision, bool Enabled, string Host, int Port, string From, string Username, string? Password, bool ClearPassword, string PublicUrl);
public sealed record MailServiceDto(string Revision, bool Enabled, string Host, int Port, string From, string Username, bool HasPassword, string PublicUrl, bool Available, Dictionary<string, string> Labels);
...
internal sealed class MailSettingsStore
{
    ...
    public MailSettingsStore(string directory, MailSettings settings, IDataProtectionProvider protection)
    {
        ...
        if (File.Exists(path)) {
            ...
            settings.Apply(new(saved.Enabled, saved.Host, saved.Port, saved.From, saved.Username,
                saved.ProtectedPassword.Length == 0 ? "" : protector.Unprotect(saved.ProtectedPassword), saved.PublicUrl));
    ...
    public MailServiceDto Read(string? locale)
    {
        lock (gate) {
            ...
            return new(revision, value.Enabled, value.Host, value.Port, value.From, value.Username, value.Password.Length > 0, value.PublicUrl, value.Ready, MailServiceLabels.For(locale));
        }
    }
    public string? Save(SaveMailSettings input)
    {
        lock (gate) {
            ...
            var value = new MailConfiguration(input.Enabled, input.Host.Trim(), input.Port, input.From.Trim(), input.Username.Trim(), password, input.PublicUrl.Trim().TrimEnd('/'));
            ...
            var document = new MailSettingsDocument(nextRevision, value.Enabled, value.Host, value.Port, value.From, value.Username,
                password.Length == 0 ? "" : protector.Protect(password), value.PublicUrl);
