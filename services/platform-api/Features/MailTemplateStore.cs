...
internal sealed class MailTemplateStore(string connectionString)
{
    ...
    {
        if(!Valid(kind,locale)||input.Revision is null||input.Subject is null||input.Introduction is null) return "invalid";
        if(!input.Reset&&(string.IsNullOrWhiteSpace(input.Subject)||input.Subject.Length>160||input.Subject.Contains('\r')||input.Subject.Contains('\n')||string.IsNullOrWhiteSpace(input.Introduction)||input.Introduction.Length>4000||kind!="notice"&&!input.Enabled))return "invalid";
