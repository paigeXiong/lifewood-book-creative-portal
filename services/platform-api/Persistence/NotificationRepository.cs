...
internal sealed partial class NotificationRepository(string connectionString)
{
 ...
 public bool SaveRule(NotificationRule r){if(!Defaults.Any(x=>x.Kind==r.Kind)||r.TitleZh is null||r.TitleEn is null||r.TitleZh.Length is <1 or >300||r.TitleEn.Length is <1 or >300||r.Level is not ("normal" or "important" or "action")||(IsBackupKind(r.Kind) ? r.Audience!="owners" : r.Audience is not ("responsible" or "allAdmins")))return false;foreach(var template in new[]{r.TitleZh,r.TitleEn})if(template.Replace("{project}","").Replace("{actor}","").Contains('{')||template.Replace("{project}","").Replace("{actor}","").Contains('}'))return false;using var c=Open();using var q=Cmd(c,"UPDATE notification_rules SET document=$doc WHERE kind=$kind AND json_extract(document,'$.version')=$version",("$kind",r.Kind),("$version",r.Version),("$doc",JsonSerializer.Serialize(r with {Version=r.Version+1},AppJsonContext.Default.NotificationRule)));return q.ExecuteNonQuery()==1;}
