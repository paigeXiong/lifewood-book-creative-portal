...
internal sealed class NotificationRepository(string connectionString)
{
 ...
 public void Initialize(){using var c=Open();using var tx=c.BeginTransaction();using var q=Cmd(c,"""
 ...
 CREATE TRIGGER notification_targets_snapshot AFTER INSERT ON notification_events BEGIN
  ...
  WHERE u.is_active=1 AND u.id<>NEW.actor_id AND (
   (NEW.kind IN ('submitted','resubmitted','customer_reply') AND (u.role IN ('admin','owner') OR (u.role='operator' AND u.id=p.assignee_user_id)) AND
 ...
 private const string Access="((e.kind='account' AND EXISTS(SELECT 1 FROM users self WHERE self.id=$user AND self.is_active=1)) OR EXISTS(SELECT 1 FROM projects p JOIN users u ON u.id=$user AND u.is_active=1 WHERE p.id=e.project_id AND ((p.owner_id=$user AND u.role='customer') OR u.role IN ('admin','owner') OR (u.role='operator' AND p.assignee_user_id=$user))))";
 ...
 public NotificationPage List(string user,string locale,long? before,string? search,string? kind,string? state,string? project,string? from,string? to,bool unread=false,bool archived=false){
  using var c=Open();using var tx=c.BeginTransaction();using var q=Cmd(c,"SELECT n.id,e.kind,e.project_id,COALESCE(json_extract(p.project_json,'$.projectName'),p.task_number,''),COALESCE(a.display_name,''),e.created_at,n.read_at IS NOT NULL,n.archived,"+State+",e.target_id"+Joins+"""
 ...
 """+" AND "+Access+" AND ($state='' OR ("+State+")=$state) ORDER BY n.id DESC LIMIT 31",("$user",user),("$before",before??long.MaxValue),("$search",(search??"")[..Math.Min(search?.Length??0,160)]),("$kind",kind??""),("$state",state??""),("$project",project??""),("$from",from??""),("$to",to??""),("$unread",unread?1:0),("$archived",archived?1:0));q.Transaction=tx;
 ...
 public bool SaveRule(NotificationRule r){if(!Defaults.Any(x=>x.Kind==r.Kind)||r.TitleZh is null||r.TitleEn is null||r.TitleZh.Length is <1 or >300||r.TitleEn.Length is <1 or >300||r.Level is not ("normal" or "important" or "action")||r.Audience is not ("responsible" or "allAdmins"))return false;foreach(var template in new[]{r.TitleZh,r.TitleEn})if(template.Replace("{project}","").Replace("{actor}","").Contains('{')||template.Replace("{project}","").Replace("{actor}","").Contains('}'))return false;using var c=Open();using var q=Cmd(c,"UPDATE notification_rules SET document=$doc WHERE kind=$kind AND json_extract(document,'$.version')=$version",("$kind",r.Kind),("$version",r.Version),("$doc",JsonSerializer.Serialize(r with {Version=r.Version+1},AppJsonContext.Default.NotificationRule)));return q.ExecuteNonQuery()==1;}
