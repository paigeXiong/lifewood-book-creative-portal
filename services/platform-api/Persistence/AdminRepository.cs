...
internal sealed class AdminRepository(string connectionString)
{
    ...
    public AdminWriteResult UpdateUser(string id, UpdateUserRequest? request, out AdminUserDto? user, string actorId="")
    {
        ...
        using var command = connection.CreateCommand();
        command.Transaction = transaction;
        command.CommandText = "UPDATE users SET display_name = $displayName, phone = $phone, role = $role, is_active = $active, session_version = session_version + CASE WHEN is_active = 1 AND $active = 0 THEN 1 ELSE 0 END, organization_id = $organizationId, updated_at = $now WHERE id = $id;";
    ...
    public AdminWriteResult AddNote(string projectId, string authorUserId, AddAdminNoteRequest? request, out AdminNoteDto? note, string? accessActorId = null)
    {
        ...
        touch.CommandText = "UPDATE projects SET workflow_updated_at = $now WHERE id = $id AND status = 'submitted';";
        touch.Parameters.AddWithValue("$now", now.ToString("O"));
        touch.Parameters.AddWithValue("$id", projectId);
        if (touch.ExecuteNonQuery() != 1) return new(AdminWriteOutcome.NotFound);
        transaction.Commit();
