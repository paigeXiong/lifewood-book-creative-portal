...
test('backup alerts have localized owner rules and open backup management',async({page})=>{
 ...
 for(const locale of ['zh-CN','en-US']){
  language=locale;await gotoInAccountLocale(page, `/${locale}/settings/notifications`);
  ...
  await gotoInAccountLocale(page, `/${locale}/notifications`);await expect(page.locator('.notification-list')).toContainText(locale==='zh-CN'?'自动备份失败，请检查备份状态':'Automatic backup failed. Check backup status.');
  ...
  await gotoInAccountLocale(page, `http://127.0.0.1:5193/${locale}/notifications`);await page.getByRole('button',{name:locale==='zh-CN'?'查看备份':'View backups',exact:true}).click();await expect(page).toHaveURL(`http://127.0.0.1:5194/${locale}/settings/backups`);await expect(page.locator('.backup-surface')).toBeVisible();
 }
});
